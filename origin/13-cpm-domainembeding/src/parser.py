...
class Parser(object):
    ...
    def init_models(self):
        ...
        if self._conf.is_shared_lstm:
            self.add_lstm(0, lstm_input_size, self._conf.lstm_layer_num)#_domain)#lstm_input_size + 2 * self._conf.lstm_hidden_dim)
    ...
    def forward(self, words, ext_words, tags, masks, domains, domain_id, word_lens, chars_i, unlabel=False):
        ...
        if unlabel == False:
            ...
            nadv_class = nadv_class.transpose(0, 1)#[batch,max_seq,domain]
            x_domain  = F.softmax(nadv_class,dim = 2)
            batch, max_seq, domain_size = x_domain.size()
            print("before multiple sofmax x_domain.size()",x_domain.size())
            # domain_all_emb[5,12]
            x_domain = torch.mm(x_domain.reshape(-1, domain_size), domain_all_emb)
            ...
            pri_lstm_out = self._lstm_layer[3](pri_lstm_out, lstm_masks, initial=None, is_training=is_training)
    ...
    def train(self):
        ...
        while True:
            ...
            if eval_cnt <= 20:
                if domain_tgt<134:
                    domain_src, domain_tgt, domain, inst_num, loss = self.train_set_label(bc, pb, zx, domain, domain_src, domain_tgt, self._train_datasets, unlabel=True)
